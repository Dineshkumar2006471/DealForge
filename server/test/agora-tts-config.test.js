...
function configure() {
  Object.assign(process.env, {
    ...
    AGORA_LLM_WEBHOOK_SECRET: 'webhook-secret',
    CLOUD_RUN_URL: 'https://service.example',
...
test('Agora agent payload contains minimal proven Deepgram ASR configuration without nested params', () => {
  ...
  const { payload } = buildAgentStartPayload(
    {
      sessionId: 'session-asr-test',
      opaqueAgoraChannel: 'df_channel',
      customerUid: 999111,
      expiresAt: new Date(Date.now() + 60_000).toISOString(),
    },
    'per-session-webhook-token',
...
test('Agora greeting uses the documented server-side Speak endpoint only after RTC readiness', async () => {
  ...
  global.fetch = async (url, options) => {
    assert.equal(
      url,
      `https://api.agora.io/api/conversational-ai-agent/v2/projects/${'a'.repeat(32)}/agents/agent-1/speak`,
    );
  ...
  try {
    assert.deepEqual(await speakAgent({ agentId: 'agent-1' }, 'Hello'), { accepted: true });
  } finally {
