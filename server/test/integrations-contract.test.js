...
const savedEnvironment = Object.fromEntries(
  ['HUBSPOT_ACCESS_TOKEN', 'CALCOM_API_KEY', 'CALCOM_EVENT_TYPE_ID', 'CALCOM_API_VERSION'].map((key) => [
    key,
    process.env[key],
  ]),
);
...
test('HubSpot field normalization rejects arbitrary CRM writes', () => {
  assert.deepEqual(normalizeProperties({ amount: 12500, dealstage: 'qualifiedtobuy' }), {
    amount: '12500',
...
test('Cal.com client pins each endpoint to its documented version, ignoring a stale runtime value', async () => {
  ...
  global.fetch = async (url, options) => {
    ...
    return new Response(JSON.stringify({ status: 'success', data: [{ id: 123, bookingFields: [] }] }), {
      status: 200,
      headers: { 'Content-Type': 'application/json' },
    });
  };
  ...
  assert.deepEqual(await calcomRequest('/event-types', { apiVersion: CALCOM_EVENT_TYPES_API_VERSION }), {
    status: 'success',
    data: [{ id: 123, bookingFields: [] }],
  });
...
test('Cal.com structured provider errors produce actionable text rather than [object Object]', async () => {
  ...
  global.fetch = async () =>
    new Response(JSON.stringify({ error: { message: 'Unsupported Cal.com API version' } }), {
      status: 404,
      headers: { 'Content-Type': 'application/json' },
    });
  await assert.rejects(
    () => calcomRequest('/bookings', { method: 'POST', apiVersion: CALCOM_BOOKINGS_CREATE_API_VERSION }),
    /Unsupported Cal\.com API version/,
  );
});
