...
test('A & B: realtimeVoiceClient.js strictly connects to GA /v1/realtime/calls and never selects legacy beta endpoints', () => {
  ...
  assert.ok(!code.includes('/v1/realtime?model='), 'Must NOT contain /v1/realtime?model=');
  assert.ok(!code.includes('OpenAI-Beta'), 'Must NOT contain OpenAI-Beta header');
  assert.ok(!code.includes('beta_api_shape_disabled'), 'Must NOT contain beta_api_shape_disabled');
});
