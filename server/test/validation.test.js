...
test('manager-created deals validate company and target ARR before a server write', () => {
  assert.deepEqual(parse(createDealSchema, { company: 'Northstar Labs', targetArr: 120000 }), {
...
test('meeting form requires safe attendee details, date, and a selected ISO slot', () => {
  ...
  assert.deepEqual(
    parse(meetingDetailsSchema, {
      sessionCredential: credential,
      attendee: { name: 'Alex Buyer', email: 'alex@example.com', timeZone: 'Asia/Kolkata' },
  ...
  assert.throws(() =>
    parse(meetingDetailsSchema, {
      sessionCredential: credential,
      attendee: { name: 'Alex', email: 'not-an-email', timeZone: 'UTC' },
      preferredDate: 'tomorrow',
    }),
  );
  assert.equal(
    parse(meetingBookingSchema, { sessionCredential: credential, slotStart: '2026-09-08T10:00:00.000+05:30' })
      .slotStart,
    '2026-09-08T10:00:00.000+05:30',
  );
});
