...
router.post('/call-links', async (req, res, next) => {
  try {
    ...
    if (/^acme\b/i.test(String(deal.data.company?.value || '')) && !deal.data.integrations?.hubspot?.dealId) {
      try {
        ...
        await writeAuditEvent({
          organizationId: req.manager.organizationId,
          dealId: input.dealId,
          sessionId: null,
          eventType: EVENT_TYPES.HUBSPOT_DEAL_LINKED,
          trigger: 'DealForge verified and linked the Acme demo HubSpot deal',
          actionResult: { hubspotDealId: verified.hubspotDealId, verified: true },
        });
      } catch (error) {
        console.warn('Acme HubSpot bootstrap verification failed:', error.message);
      }
    }
    ...
    res.status(201).json({
      ...
      callUrl: `${publicAppUrl}/call.html?link=${encodeURIComponent(linkToken)}`,
    });
  } catch (error) {
    next(error);
  }
  ...
  try {
    ...
    await db.runTransaction(async (tx) => {
      ...
      tx.create(db.collection('auditEvents').doc(uuidv4()), {
        ...
        trigger: 'Manager created a new Deal Workspace',
        actionResult: { company: input.company, targetArr: input.targetArr, verified: true },
        timestamp: createdAt,
      });
    });
...
router.post('/deals/:dealId/integrations/hubspot/link', async (req, res, next) => {
  try {
    ...
    await writeAuditEvent({
      ...
      sessionId: null,
      eventType: EVENT_TYPES.HUBSPOT_DEAL_LINKED,
      trigger: 'Manager linked verified HubSpot staging deal',
      actionResult: { hubspotDealId: verified.hubspotDealId, verified: true },
    });
...
router.post('/deals/:dealId/integrations/hubspot/booking-sync', async (req, res, next) => {
  try {
    ...
    await writeAuditEvent({
      ...
      dealId: req.params.dealId,
      sessionId: null,
      eventType: EVENT_TYPES.HUBSPOT_BOOKING_SYNC_UPDATED,
      trigger: `Manager ${enabled ? 'enabled' : 'disabled'} verified booking-to-CRM sync`,
      actionResult: { enabled, verified: enabled },
    });
    res.json({ enabled });
  } catch (error) {
    next(error);
...
router.post('/calls/:sessionId/revoke', async (req, res, next) => {
  try {
    const { revokeSession } = require('../lib/calls/callSessions');
    const ref = sessionRef(req.params.sessionId);
    const doc = await ref.get();
    if (!doc.exists || doc.data().organizationId !== req.manager.organizationId) {
      throw new HttpError(404, 'Call session not found');
    ...
    await writeAuditEvent({
      ...
      eventType: EVENT_TYPES.CALL_ENDED,
      trigger: agentStopped ? 'Manager revoked customer call link' : 'Manager revoked link; Agora cleanup pending',
      actionResult: { agentStopped },
    });
    res.status(agentStopped ? 200 : 202).json({
      sessionId: req.params.sessionId,
      status: agentStopped ? 'REVOKED' : 'REVOKED_WITH_AGENT_CLEANUP_ERROR',
      agentStopped,
    });
  } catch (error) {
    next(error);
...
router.post('/approvals/:approvalId/resolve', async (req, res, next) => {
  try {
    res.json(
      await resolveApproval(req.params.approvalId, parse(approvalResolutionSchema, req.body).decision, req.manager),
    );
  } catch (error) {
    next(error);
  }
...
router.get('/integrations/status', async (req, res) => {
  ...
  const agora =
    configured('AGORA_APP_ID') &&
    configured('AGORA_CUSTOMER_ID') &&
    configured('AGORA_CUSTOMER_SECRET') &&
    configured('CLOUD_RUN_URL')
      ? 'AVAILABLE'
      : 'NOT CONFIGURED';
...
router.get('/agent-status', async (req, res) => {
  ...
  const integrations = {
    agora: Boolean(process.env.AGORA_APP_ID && process.env.ELEVENLABS_API_KEY),
    gemini: Boolean(process.env.GCP_PROJECT_ID && process.env.GEMINI_MODEL),
    firestore: true,
  ...
  const events = await db
    ...
    .get();
  res.json({
    voice: integrations.agora ? 'AVAILABLE' : 'NOT CONFIGURED',
    reasoning: integrations.gemini ? 'AVAILABLE' : 'NOT CONFIGURED',
    memory: integrations.firestore ? 'CONNECTED' : 'ERROR',
    policy: 'ENFORCING',
    integrations,
    recentActions: events.docs.map((doc) => ({ id: doc.id, ...doc.data() })),
  });
});
