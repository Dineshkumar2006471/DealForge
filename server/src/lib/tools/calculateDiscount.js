...
async function calculateDiscount(args, context) {
  ...
  if (requestedPct <= 18) {
    ...
    await recordSignal(dealId, organizationId, {
      sessionId: context.sessionId,
      type: 'DISCOUNT_ACCEPTED',
      requestedPct,
      offeredPct: requestedPct,
      status: 'APPROVED',
      turn_stated: context.turnNumber,
      context: 'Discount within autonomous limit',
    });
  ...
  if (requestedPct <= 25) {
    ...
    await recordSignal(dealId, organizationId, {
      sessionId: context.sessionId,
      type: context.approvedReplay ? 'DISCOUNT_ACCEPTED' : 'DISCOUNT_REQUESTED',
      requestedPct,
      offeredPct: context.approvedReplay ? requestedPct : 18,
      tradeOffs: proposal?.tradeOffs || [],
      status: context.approvedReplay ? 'APPROVED' : 'PENDING_APPROVAL',
      urgency: 'unknown',
      turn_stated: context.turnNumber,
      context: context.approvedReplay ? 'Exact manager-approved discount executed' : 'Manager approval required',
    });
    ...
    return {
      ...
      tradeoff: proposal,
      message: context.approvedReplay
        ? `${requestedPct}% discount approved by your manager.`
        : `${requestedPct}% exceeds autonomous limit (18%). Approval request created. Counter-offer: 18% + concessions.`,
    };
  }
  ...
  await recordSignal(dealId, organizationId, {
    ...
    status: 'REJECTED',
    turn_stated: context.turnNumber,
    context: 'Discount exceeds policy maximum',
  });
...
registerTool('calculate_discount', calculateDiscount, {
  description:
    'Calculate and apply a discount percentage. Policy enforced: ≤18% auto, 18-25% needs approval, >25% rejected.',
  parameters: {
    type: 'object',
    properties: {
      requested_pct: {
