...
async function calcomRequest(path, { method = 'GET', body, apiVersion } = {}) {
  ...
  if (!response.ok) {
    throw new Error(
      `Cal.com ${method} ${path} failed (${response.status}): ${calcomErrorMessage(payload).slice(0, 180)}`,
    );
  }
  return payload;
}
...
async function configuredEventType() {
  ...
  const eventType = Array.isArray(response.data)
    ? response.data.find((item) => Number(item.id) === config.eventTypeId)
    : null;
  ...
  const unsupported = (eventType.bookingFields || []).filter(
    (field) => field?.required && !['name', 'email'].includes(field.type),
  );
...
function operationIdFor(args, context) {
  return crypto
    .createHash('sha256')
    .update(
      `${context.sessionId}:${context.dealId}:calcom:${args.preferred_date}:${args.attendee.email}:${args.meeting_type}`,
    )
    .digest('hex');
}
...
function availableAt(slotData, preferredDate) {
  ...
  const slots = Object.values(slotData || {})
    .flat()
    .filter(Boolean);
...
async function probeCalcom() {
  ...
  try {
    ...
    return {
      status: /inactive|unsupported/i.test(error.message) ? 'DEGRADED' : 'ERROR',
      verified: false,
      error: error.message,
    };
  }
...
function flattenSlots(slotData) {
  return Object.values(slotData || {})
    .flat()
    .map((slot) => (typeof slot === 'string' ? { start: slot } : slot))
  ...
  try {
    ...
    if (existing.exists && existing.data().status === 'RUNNING') {
      return {
        booked: false,
        verified: false,
        externalStatus: 'BOOKING_IN_PROGRESS',
        error: 'A booking for this exact request is already in progress.',
      };
    }
    ...
    const slots = await getAvailableSlots({
      preferredDate: new Date(args.preferred_date).toISOString().slice(0, 10),
    ...
    const created = await calcomRequest('/bookings', {
      method: 'POST',
      apiVersion: CALCOM_BOOKINGS_CREATE_API_VERSION,
      body: {
        start: args.preferred_date,
    ...
    const verifiedBooking = await calcomRequest(`/bookings/${encodeURIComponent(bookingId)}`, {
      apiVersion: CALCOM_BOOKING_READ_API_VERSION,
    });
    ...
    const result = {
      ...
      meetingUrl: verifiedBooking.data.location || created.data?.location || null,
    };
    ...
    await writeAuditEvent({
      ...
      trigger: 'Cal.com booking created and verified',
      actionResult: { bookingId: String(bookingId), verified: true },
    });
    return result;
  } catch (err) {
    ...
    await operationRef
      .set({ status: 'FAILED', error: result.error, updatedAt: new Date().toISOString() }, { merge: true })
      .catch(() => {});
    return result;
  }
}
...
registerTool('book_meeting', bookMeeting, {
  ...
  agentVisible: false,
  description:
    'Request a verified follow-up meeting. It returns an error until a verified server-side scheduling integration is enabled.',
  parameters: {
    type: 'object',
    properties: {
      meeting_type: {
        type: 'string',
        description: 'Type of meeting: enterprise_demo, technical_review, executive_briefing',
      },
