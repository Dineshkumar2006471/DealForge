...
const requestRef = (sessionId, requestId) =>
  db.collection('callSessions').doc(sessionId).collection('meetingRequests').doc(requestId);
...
async function requestMeetingDetails({ organizationId, dealId, sessionId, meetingType }) {
  ...
  if (created) {
    await writeAuditEvent({
      ...
      dealId,
      sessionId,
      eventType: EVENT_TYPES.MEETING_DETAILS_REQUESTED,
      trigger: 'Agent requested secure meeting details form',
      actionResult: { meetingType, verified: true },
    });
  }
  return {
    verified: true,
    requestId,
    status: created ? 'DETAILS_REQUIRED' : 'ALREADY_REQUESTED',
    message: 'The secure meeting form is ready for the customer.',
  };
}
...
async function requireRequest(session, requestId, statuses) {
  ...
  if (
    ...
    request.dealId !== session.dealId ||
    request.sessionId !== session.sessionId
  ) {
    throw new HttpError(404, 'Meeting request not found');
  }
  if (!statuses.includes(request.status)) {
    throw new HttpError(409, `Meeting request is ${String(request.status || 'unavailable').toLowerCase()}`);
  }
...
async function findMeetingSlots(session, requestId, { attendee, preferredDate }) {
  ...
  await writeAuditEvent({
    ...
    eventType: EVENT_TYPES.MEETING_SLOTS_READY,
    trigger: 'Cal.com availability verified for meeting request',
    actionResult: { requestId, slotCount: slots.length, verified: true },
  });
...
async function confirmMeeting(session, requestId, slotStart) {
  ...
  if (!request.attendee || !Array.isArray(request.availableSlots)) {
    throw new HttpError(409, 'Meeting details must be submitted before booking');
  }
  ...
  let result;
  try {
    result = await bookMeeting(
      { meeting_type: request.meetingType, preferred_date: slotStart, attendee: request.attendee },
      { organizationId: session.organizationId, dealId: session.dealId, sessionId: session.sessionId, turnNumber: 0 },
    );
  } catch (bookingErr) {
    await failOperation(opId, bookingErr);
    await ref.set(
      { status: 'BOOKING_FAILED', result: { error: bookingErr.message }, updatedAt: now() },
      { merge: true },
    );
  ...
  const crm = await syncBookingToHubspot({
    organizationId: session.organizationId,
    dealId: session.dealId,
    sessionId: session.sessionId,
    booking: { bookingId: result.bookingId, start: slotStart, meetingUrl: result.meetingUrl || null },
  });
  await completeOperation(opId, {
    externalRecordId: result.bookingId,
    externalUrl: result.meetingUrl || null,
    result: { ...result, crm },
  });
