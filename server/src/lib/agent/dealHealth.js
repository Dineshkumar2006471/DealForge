...
function calculateDealHealth(deal = {}) {
  ...
  if (value(deal, 'timeline')) {
    score += 10;
    factors.push({ name: 'Timeline', contribution: 10, detail: 'Customer timeline recorded' });
  }
  if (value(deal, 'budget')) {
    score += 10;
    factors.push({ name: 'Budget', contribution: 10, detail: 'Budget signal recorded' });
  }
  if (value(deal, 'pain')) {
    score += 10;
    factors.push({ name: 'Pain', contribution: 10, detail: 'Customer pain recorded' });
  }
  if (value(deal, 'competitor')) {
    score -= 8;
    factors.push({ name: 'Competitor risk', contribution: -8, detail: 'Competitive evaluation recorded' });
  }
  const pending = (deal.discountLedger || []).some((entry) => entry.result === 'PENDING_APPROVAL');
  if (pending) {
    score -= 6;
    factors.push({ name: 'Pending commercial approval', contribution: -6, detail: 'Manager decision required' });
  }
