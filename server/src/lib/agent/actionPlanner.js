...
function planIdFor({ sessionId, dealId, action, turnNumber = 0 }) {
  return crypto.createHash('sha256').update(`${sessionId}:${dealId}:${action}:${turnNumber}`).digest('hex');
}
...
async function createActionPlan({ organizationId, dealId, sessionId, turnNumber, nextBestAction }) {
  ...
  const plan = {
    ...
    turnNumber,
    nextBestAction,
    steps: nextBestAction.requiredTool ? [{ tool: nextBestAction.requiredTool, status: 'PROPOSED' }] : [],
    status: 'PROPOSED',
  ...
  await writeAuditEvent({
    ...
    dealId,
    sessionId,
    eventType: EVENT_TYPES.ACTION_PLAN_CREATED,
    trigger: `Autonomous action plan: ${nextBestAction.action}`,
    actionResult: { planId, verified: false },
  });
  return plan;
}
