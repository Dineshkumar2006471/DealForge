...
async function refreshAutonomy({ organizationId, dealId, sessionId, turnNumber = 0 }) {
  ...
  const approvals = await db.collection('approvals').where('organizationId', '==', organizationId).where('dealId', '==', dealId).where('status', 'in', ['PENDING', 'APPROVED', 'EXECUTING']).get();
  ...
  await updateDealHealth(dealId, dealHealth, organizationId);
  await updateNextBestAction(dealId, nextBestAction, organizationId);
