...
async function refreshAutonomy({ organizationId, dealId, sessionId, turnNumber = 0 }) {
  ...
  const nextBestAction = evaluateNextBestAction(deal, {
    pendingApprovals,
    availableTools: ['update_deal_state', 'calculate_discount', 'book_meeting'],
  });
  ...
  await writeAuditEvent({
    ...
    dealId,
    sessionId,
    eventType: EVENT_TYPES.DEAL_HEALTH_CALCULATED,
    trigger: 'Verified state refreshed autonomous assessment',
    actionResult: { score: dealHealth.score, riskLevel: dealHealth.riskLevel, verified: true },
  });
  await writeAuditEvent({
    organizationId,
    dealId,
    sessionId,
    eventType: EVENT_TYPES.NEXT_BEST_ACTION_UPDATED,
    trigger: nextBestAction.reason,
    actionResult: { action: nextBestAction.action, authority: nextBestAction.authority, verified: true },
  });
