...
function createTradeoffProposal(requestedPct) {
  ...
  if (requested <= 18) {
    return {
      ...
      offeredPct: requested,
      tradeOffs: [],
      authority: 'ACT',
      reason: 'Within the autonomous discount limit.',
    };
  }
  return {
    requestedPct: requested,
    offeredPct: 18,
    tradeOffs: getConcessions(2).map((item) => ({ id: item.id, label: item.name })),
    authority: 'APPROVAL',
    reason: 'The request exceeds the autonomous discount limit; offer value before pure discount.',
  };
}
