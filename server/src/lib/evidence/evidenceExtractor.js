...
function extractEvidenceSignals(text) {
  ...
  const companyMatch = normalized.match(
    /(?:we're|we are|from|company is)\s+([A-Z][A-Za-z0-9\s]{2,30}?)(?:\.|,|\s+we\b|\s+and\b|$)/i,
  );
  if (companyMatch) {
    const raw = companyMatch[1].trim();
    const nonCompanyVerbs =
      /^(a|an|the|looking|trying|having|experiencing|evaluating|considering|testing|using|reviewing|planning|ready|hoping|interested|scheduling|schedule|review|meeting|pending)\b/i;
    const nonCompanyKeywords =
      /(salesforce|hubspot|gong|outreach|tomorrow|review|meeting|discount|evaluating|pending)/i;
  ...
  const teamMatch = normalized.match(
    /(?:have|about|around|with|got)?\s*(\d{1,5})\s*(?:sales\s*(?:reps?|representatives?)|reps?|users?|seats?|agents?|people)\b/i,
  );
  if (teamMatch) {
  ...
  if (
    /(?:main problem|pain point|challenge|struggling with|biggest issue|main pain)/i.test(normalized) ||
    /inbound lead qualification/i.test(normalized)
  ) {
    const painValue = /inbound lead qualification/i.test(normalized)
      ? 'Inbound lead qualification'
      : normalized.match(/(?:problem|challenge|issue|is)\s*(?:is\s*)?([^.,?!]+)/i)?.[1]?.trim() ||
        'Inbound lead qualification';
  ...
  // 4. Metrics (MEDDIC)
  const metricsMatch = normalized.match(
    /(\d{1,3}%\s*(?:of\s*)?(?:rep\s*capacity|capacity|time|pipeline|conversion|spend|qualification)[^.,?!]*)/i,
  );
  if (metricsMatch) {
  ...
  const buyerMatch = normalized.match(
    /(?:economic buyer|approv(?:ed|al)|sign[\s-]?off|decid(?:e|es|ing)|decision maker)\s*(?:is|from|by|will be)?\s*(?:the\s+)?(VP\s+of\s+Sales|Chief\s+Revenue\s+Officer|CRO|CEO|CFO|VP\s+Sales|Vice\s+President)/i,
  );
  ...
  // 6. Competitor
  const competitorMatch = normalized.match(
    /(?:evaluating|comparing|looking at|considering|competitor|vs|against)\s+([A-Za-z0-9\s]+?)(?:too|also|as well|[.,]|$)/i,
  );
  if (competitorMatch) {
  ...
  const budgetMatch = normalized.match(
    /(?:budget\s*(?:is|of)?\s*)?(₹?\s*\d+(?:\s*[–-]\s*₹?\s*\d+)?\s*(?:lakh|lakhs|L)\b[^.,?!]*)/i,
  );
  ...
  const timelineMatch = normalized.match(
    /(?:decision|implement(?:ation)?|rollout|close|timeline)\s*(?:is\s*)?(this\s+month|next\s+month|\d+\s*(?:weeks?|months?)[^.,?!]*)/i,
  );
  ...
  if (
    /(?:decision criteria|criteria|priorities|must-have|requirements)/i.test(normalized) ||
